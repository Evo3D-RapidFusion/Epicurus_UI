...
function initializeExpansionVariables() {
  ...
  if (bedExpansionEnabled) {
    ...
    expansionCodeURL = null;
    expansionConnectURL = null;
  }
}
...
async function connectToExpansionRRF(password = "reprap", retryCount = 0, tryFallback = true) {
  ...
  // Ensure expansion variables are initialized
  if (!expansionConnectURL) {
    initializeExpansionVariables();
    if (!expansionConnectURL) {
      console.warn("Expansion controller URLs not initialized");
      return null;
    }
  }
...
function updateObjectModel() {
  return new Promise(async (resolve, reject) => {
    try {
      // Fetch from main controller only (expansion controller commented out)
      // console.log("Fetching data from both main controller and expansion controller...");
      console.log("Fetching data from main controller...");
      
      // Detect mode for main controller
      const mainMode = await detectDuetMode();
      
      // Fetch data from main controller based on mode
      let mainDataPromise;
      ...
      // Fetch data from main controller
      let mainData;
      try {
        mainData = await mainDataPromise;
      } catch (mainError) {
        console.error("Main controller failed:", mainError);
        throw mainError;
      }
      
      // Fetch expansion data if enabled
      let expansionData = null;
      if (bedExpansionEnabled && expansionDataPromise) {
        try {
          expansionData = await expansionDataPromise;
        } catch (expansionError) {
          console.warn("Expansion controller data fetch failed:", expansionError);
      ...
      console.log(`Fetched main data using ${mainMode} mode${bedExpansionEnabled ? ', expansion data using heat endpoint' : ''}`);

      // Extract data from controllers
      const mainActualData = mainData.result || mainData;
      const expansionActualData = expansionData ? (expansionData.result || expansionData) : null;
      ...
      // Create heater arrays from main controller
      ...
      // Add expansion bed heaters (4-9) from heater indices 8-13 if expansion is enabled
      if (bedExpansionEnabled && expansionHeatData && expansionHeatData.heaters && expansionHeatData.heaters.length > 8) {
      ...
      function updateUIdata(
        ...
        switch (objectDescription) {
          case "extruderHeaters":
            ...
            return outputData;
          case "bedHeaters":
            // For bed heaters, we need to map through the bedHeaters array to find actual heaters
            // configuredBedHeaters contains [bedHeaterIndex, bedPosition] pairs from findHeaters()
            let bedOutputData = [];
            configuredBedHeaters.forEach(([bedHeaterIndex, bedPosition]) => {
              // bedHeaterIndex is the actual heater index in the heaters array
              if (bedHeaterIndex < targetObject.length && targetObject[bedHeaterIndex]) {
                bedOutputData.push(targetObject[bedHeaterIndex][targetProperty]);
              } else {
                bedOutputData.push(""); // Empty for missing heaters
              }
            });
            elementsWithClass = document.querySelectorAll(targetClass);
            outputData = updateText(elementsWithClass, bedOutputData, endText);
            return outputData;
          case "chamberHeaters":
            elementsWithClass = document.querySelectorAll(targetClass);
            outputData = updateText(elementsWithClass, outputData, endText);
      ...
      const allConfiguredBedHeaters = findHeaters(heatData.bedHeaters || []);
      ...
      // Limit to main controller beds only (0-3) - expansion controller not available
      const configuredBedHeaters = allConfiguredBedHeaters.slice(0, 4); // Only take first 4 beds (0-3)

      // Show configured main controller bed heaters (0-3)
      const mainConfiguredBedHeaters = configuredBedHeaters; // Already limited to beds 0-3
...
async function sendExpansionGcode(gcode) {
  ...
  while (true) {
    try {
      ...
      // Check for unknown variable error in the response text
      if (response && typeof response === "string" && response.includes("Error: unknown variable")) {
...
function toggleHeaterStates(heaterState, heaterIndex) {
  ...
  } else if (!bedExpansionEnabled && heaterIndex >= 8 && heaterIndex <= 13) {
    console.warn(`Bed heaters 4-9 (indices 8-13) are not available - bed expansion is disabled`);
    return; // Exit early to prevent sending invalid commands
  }
...
// FUNCTION: configureBedHeaters (main controller only - expansion controller commented out)
function configureBedHeaters(mode, configuredBedHeaters) {
  let mainGcodeString = "";
  // let expansionGcodeString = ""; // COMMENTED OUT - Expansion controller not available
  ...
  configuredBedHeaters.forEach((heater, index) => {
    ...
    if (heaterElement && activeElement) {
      ...
      // Route bed heaters 0-3 to main controller only (expansion controller commented out)
      if (index < 4) {
  ...
  // Send commands to main controller
...
document.getElementById('software-version').textContent = 'v3.3';
...
window.addEventListener("load", () => {
  ...
  requestAnimationFrame(() => {
    setTimeout(() => {
      ...
      // Initialize Controller IP dropdown
      ...
      if (ipSelect) {
        ...
        ipSelect.addEventListener("change", function() {
          ...
          if (newIP !== duetIP) {
            ...
            connectToRRF().catch(error => {
              console.error("Failed to connect with new IP:", error);
            });
          }
        });
      }
      ...
      if (bedExpansionState === "on") {
        ...
        document.getElementById("bed-expansion-off").click();
      }
  ...
  document.getElementById(buttonId).addEventListener("click", () => {
    switch (buttonId) {
      ...
      case "bed-heaters-off":
        ...
        // Send explicit off commands to all bed heaters on main controller for safety
        ...
        break;
      case "preheat-bed":
        // Use the existing configureBedHeaters function (main controller only)
        ...
        console.log("Preheat bed command sent to main controller");
        break;
      case "emergency-stop":
        sendGcode("M112");
        if (SettingsManager.get("bedExpansionState") === "on") {
      ...
      case "reset-machine":
        sendGcode("M999");
        // sendExpansionGcode("M999"); // COMMENTED OUT - Expansion controller not available
        break;
      case "confirmYes":
        if (spindleRunning == true) {
          let spindleSpeed = document.getElementById("speedValue").textContent;
      ...
      case "bed-expansion-on":
        ...
        connectToExpansionRRF().catch(error => {
          console.warn("Failed to connect to expansion controller:", error);
        });
        console.log("Bed expansion enabled");
        break;
      case "bed-expansion-off":
        ...
        console.log("Bed expansion disabled");
        break;
...
function setTemp(tabpane, buttonIndex) {
  ...
  let [activeTemp, preheatTemp, heater] = (() => {
    switch (tabpane) {
      ...
      // COMMENTED OUT - Expansion controller not available in this setup
      // Beds 4-9 (P8-P13) are not available on main controller
      // case ".tab-pane-bed4":
      //   return [
  ...
  // Guard: Check if heater/activeTemp/preheatTemp are undefined (bed4-bed9 cases are commented out)
  if (!heater || !activeTemp || !preheatTemp) {
    console.warn(`Bed heaters 4-9 are not available - expansion controller is not configured`);
    return; // Exit early if bed4-bed9 tab was accessed
  }
  ...
  // Additional guard: Prevent sending commands for expansion beds (P8-P13) since expansion controller is not available
  if (heater >= "P8" && heater <= "P13") {
    console.warn(`Bed heaters P8-P13 (bed4-bed9) are not available - expansion controller is not configured`);
    return; // Exit early to prevent sending invalid commands
  }
...
document
  ...
  .forEach((element, index) => {
    element.addEventListener("click", () => {
      ...
      // Preheat Bed Heaters (main controller only - expansion controller commented out)
      let mainGcodeString = "";
      // let expansionGcodeString = ""; // COMMENTED OUT - Expansion controller not available
      ...
      // Send commands to main controller only
      if (mainGcodeString.trim()) {
